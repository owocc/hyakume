...
function rowToApp(row: schema.AppSelect | Record<string, unknown>): AppItem {
  ...
  return {
    ...
    rating_count: String(row.rating_count ?? "0"),
...
export async function createTask(task: PipelineTaskItem): Promise<PipelineTaskItem> {
  ...
  await (database as NeonHttpDatabase<typeof schema>)
    ...
    .onConflictDoUpdate({
      ...
      setWhere: and(eq(tasksTable.user_id, task.user_id), eq(tasksTable.url, task.url)),
      set: {
...
export async function getTaskById(id: string): Promise<PipelineTaskItem | null> {
  ...
  return {
    ...
    status: r.status as PipelineTaskItem["status"],
    step: Number(r.step),
